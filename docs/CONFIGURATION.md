...
1. [`plugins/codex-fusion-drive/config/default.json`](../plugins/codex-fusion-drive/config/default.json)
...
The authoritative field catalog is [`plugins/codex-fusion-drive/schemas/config.schema.json`](../plugins/codex-fusion-drive/schemas/config.schema.json). The MCP tools expose the same surface:
...
The JSON files under [`plugins/codex-fusion-drive/examples`](../plugins/codex-fusion-drive/examples) are override fragments intended to be deep-merged with the shipped default:
...
```bash
export RELENTLESS_INCEPTION_CONFIG="$PWD/plugins/codex-fusion-drive/examples/xai-grok45-direct.json"
...
```bash
python3 -m json.tool plugins/codex-fusion-drive/config/default.json >/dev/null
python3 -m json.tool plugins/codex-fusion-drive/schemas/config.schema.json >/dev/null
for file in plugins/codex-fusion-drive/examples/*.json; do python3 -m json.tool "$file" >/dev/null; done
```

Validate the schema and shipped default when `jsonschema` is available:

```bash
PYTHONPATH=plugins/codex-fusion-drive python3 - <<'PY'
import json
from pathlib import Path
from jsonschema import Draft202012Validator

root = Path("plugins/codex-fusion-drive")
...
for path in sorted(Path("plugins/codex-fusion-drive/examples").glob("*.json")):
