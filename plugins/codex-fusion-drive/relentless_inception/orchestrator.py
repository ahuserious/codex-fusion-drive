...
def _aggregate_continuation_ledger(
    source_usage: Mapping[str, Any],
    child_ledger: Mapping[str, Any],
) -> Dict[str, Any]:
    """Report source and child consumption without forging one receipt chain."""
    ...
    aggregate: Dict[str, Any] = {
        "schema_version": 1,
        "scope": "source_plus_child",
        "receipt_chains_separate": True,
    }
    for field_name in integer_fields:
        source_value = source_usage.get(field_name, 0)
        child_value = child_ledger.get(field_name, 0)
        if (
            not isinstance(source_value, int)
            or isinstance(source_value, bool)
            or source_value < 0
            or not isinstance(child_value, int)
            or isinstance(child_value, bool)
            or child_value < 0
        ):
            raise ConfigError(
                f"Continuation accounting field {field_name} must be a nonnegative integer"
            )
        aggregate[field_name] = source_value + child_value

    aggregate["attempts"] = aggregate["calls"]
    aggregate["total_tokens"] = (
        aggregate["input_tokens"] + aggregate["output_tokens"]
    )
    source_cost = source_usage.get("known_cost_usd", 0.0)
    child_cost = child_ledger.get("known_cost_usd", 0.0)
    source_wall = source_usage.get("wall_seconds", 0.0)
    child_wall = child_ledger.get("wall_seconds", 0.0)
    for field_name, value in (
        ("source known_cost_usd", source_cost),
        ("child known_cost_usd", child_cost),
        ("source wall_seconds", source_wall),
        ("child wall_seconds", child_wall),
    ):
        if (
            ...
            raise ConfigError(
                f"Continuation accounting field {field_name} must be nonnegative and finite"
            )
    aggregate["known_cost_usd"] = float(source_cost) + float(child_cost)
    aggregate["wall_seconds"] = float(source_wall) + float(child_wall)
    ...
    for scope_name, scope in (
        ...
        scoped_cost = scope.get("provider_cost_usd", {})
        if not isinstance(scoped_cost, Mapping):
            raise ConfigError(
                f"Continuation {scope_name} provider_cost_usd must be an object"
            )
        for provider_name, cost_value in scoped_cost.items():
            if (
                ...
                raise ConfigError(
                    f"Continuation {scope_name} provider cost entry is invalid"
                )
            provider_cost[provider_name] = (
                provider_cost.get(provider_name, 0.0) + float(cost_value)
            )
    aggregate["provider_cost_usd"] = provider_cost
    warnings: List[str] = []
    for scope in (source_usage, child_ledger):
        scoped_warnings = scope.get("warnings", [])
        if not isinstance(scoped_warnings, list) or any(
            not isinstance(warning, str) or not warning
            for warning in scoped_warnings
        ):
            raise ConfigError("Continuation accounting warnings must be strings")
        for warning in scoped_warnings:
            if warning not in warnings:
                warnings.append(warning)
    aggregate["warnings"] = warnings
    aggregate["accounting_failure"] = child_ledger.get("accounting_failure")
    aggregate["stop_reason"] = child_ledger.get("stop_reason")
    attempt_entries = child_ledger.get("attempt_entries")
    response_entries = child_ledger.get("entries")
    if not isinstance(attempt_entries, list) or not isinstance(response_entries, list):
        raise ConfigError(
            "Continuation child ledger must contain attempt and response arrays"
        )
    receipted_attempt_ids = {
        entry.get("attempt_id")
        for entry in response_entries
        if isinstance(entry, Mapping)
    }
    unreceipted_attempt_ids: List[str] = []
    for attempt in attempt_entries:
        attempt_id = attempt.get("attempt_id") if isinstance(attempt, Mapping) else None
        if not isinstance(attempt_id, str) or not attempt_id:
            raise ConfigError("Continuation child attempt has no valid attempt_id")
        if attempt_id not in receipted_attempt_ids:
            unreceipted_attempt_ids.append(attempt_id)
    aggregate["unreceipted_attempt_count"] = len(unreceipted_attempt_ids)
    aggregate["unreceipted_attempt_ids"] = unreceipted_attempt_ids
    if unreceipted_attempt_ids:
        unresolved_warning = (
            "Child provider attempts without response receipts may still be billable."
        )
        if unresolved_warning not in warnings:
            warnings.append(unresolved_warning)
    aggregate["source"] = dict(source_usage)
    aggregate["child"] = dict(child_ledger)
    return aggregate
...
class FusionOrchestrator:
        ...
        *,
        ...
        try:
            ...
            handoff = build_handoff(
                ...
                gates=profile.get("gates", {}),
                native_claude=self.config.get("native_claude", {}),
            )
    ...
    def _continue_fuse_validated(
        ...
        *,
        ...
        profile_name: Optional[str] = None,
        run_id: Optional[str] = None,
        source_integrity_check: Optional[Callable[[], None]] = None,
    ) -> FusionResult:
        """Continue after a validated failed judge without redispatching panels.

        ``source_binding`` is produced by the Fusion Drive continuation
        validator. Source-bound panel receipts remain source-bound and are
        persisted as an explicit import; this run owns only the judge retry,
        synthesis, and gate receipts.
        """

        from codex_fusion_drive.continuation import (
            validated_execution_source_snapshot,
        )

        # Snapshots and arbitrary callbacks are not authority to dispatch.
        # Only a sealed binding produced by the full source validator is.
        del source_snapshot, source_integrity_check
        if not task.strip():
            raise ConfigError("Fusion continuation task must not be empty")
        if not isinstance(source_profile_name, str) or not source_profile_name:
            raise ConfigError(
                "Fusion continuation requires its bound source profile"
            )
        ...
        source_snapshot = validated_execution_source_snapshot(
            source_binding,
            reverify=True,
            task=task,
            context=context,
            ...
            current_schema_v2_sha256=None,
            translated_profile_name=selected_profile_name,
            translated_engine_sha256=canonical_json_hash(self.config),
            child_run_id=run_id,
        )
        ...
        missing_snapshot_fields = required_snapshot_fields.difference(source_snapshot)
        if missing_snapshot_fields:
            raise ConfigError(
                "Fusion continuation source snapshot is missing: "
                + ", ".join(sorted(missing_snapshot_fields))
            )
        lineage = source_snapshot.get("lineage")
        reports_value = source_snapshot.get("reports")
        source_usage = source_snapshot.get("source_usage")
        residual_budget = source_snapshot.get("residual_budget")
        if not isinstance(lineage, Mapping):
            raise ConfigError("Fusion continuation lineage must be an object")
        if not isinstance(reports_value, list) or any(
            not isinstance(report, Mapping) for report in reports_value
        ):
            raise ConfigError("Fusion continuation reports must be an array of objects")
        if not isinstance(source_usage, Mapping):
            raise ConfigError("Fusion continuation source_usage must be an object")
        if not isinstance(residual_budget, Mapping):
            raise ConfigError("Fusion continuation residual_budget must be an object")

        profile = active_profile(self.config, profile_name)
        self._assert_external_provider_access(profile)
        self._bind_selected_profile(selected_profile_name)
        fusion_config = profile["fusion"]
        expected_panel_seats = list(fusion_config.get("panel", []))
        for optional_seat_name in fusion_config.get("optional_panel", []):
            optional_seat = self._seat_config(str(optional_seat_name))
            optional_provider = self.config.get("providers", {}).get(
                optional_seat.get("provider"), {}
            )
            if (
                optional_seat.get("enabled", True) is True
                and isinstance(optional_provider, Mapping)
                and optional_provider.get("enabled", True) is True
            ):
                expected_panel_seats.append(str(optional_seat_name))
        expected_panel_seats = expected_panel_seats[
            : int(fusion_config.get("max_panel_seats", len(expected_panel_seats)))
        ]
        imported_seats = [report.get("seat_name") for report in reports_value]
        if (
            len(imported_seats) != len(set(imported_seats))
            or set(imported_seats) != set(expected_panel_seats)
            or any(report.get("status") != "completed" for report in reports_value)
        ):
            raise ConfigError(
                "Fusion continuation requires one completed imported report for "
                "every configured panel seat"
            )

        detached_snapshot = json.loads(
            json.dumps(source_snapshot, sort_keys=True, ensure_ascii=False)
        )
        snapshot_sha256 = canonical_json_hash(detached_snapshot)
        store = RunStore(
            task,
            self.config,
            run_id,
        ...
        budget = BudgetTracker(residual_budget)
        accounting_initialized = False

        def persist_continuation_accounting(
            status: str,
            *,
            child_outcome: Optional[str] = None,
        ) -> tuple[Dict[str, Any], Dict[str, Any]]:
            """Persist separate source and child scopes before reporting their aggregate."""

            child_ledger = store.write_budget_snapshot(budget)
            aggregate_ledger = _aggregate_continuation_ledger(
                source_usage,
                child_ledger,
            )
            accounting_receipt = {
                ...
                "unreceipted_attempt_count": aggregate_ledger[
                    "unreceipted_attempt_count"
                ],
                "unreceipted_attempt_ids": aggregate_ledger[
                    "unreceipted_attempt_ids"
                ],
            }
            store.write_json("continuation-accounting.json", accounting_receipt)
            store.mark_stage(
                "continuation-accounting",
                status,
                "continuation-accounting.json",
            )
            return child_ledger, aggregate_ledger

        def refresh_failed_accounting(
            status: str,
            primary_error: BaseException,
        ) -> None:
            """Best-effort refresh without replacing the orchestration failure."""

            try:
                persist_continuation_accounting(
                    status,
                    child_outcome=status,
                )
            except Exception as accounting_error:
                # The provider/orchestration exception remains primary. The
                # child ledger is independently persisted by _call before and
                # after provider transport, so an accounting-receipt write
                # failure cannot erase billable child evidence.
                note = (
                    "Continuation accounting refresh also failed with "
                    f"{type(accounting_error).__name__}."
                )
                setattr(primary_error, "continuation_accounting_note", note)
                add_note = getattr(primary_error, "add_note", None)
                if callable(add_note):
                    add_note(note)

        try:
            if store.exists("panel-import.json"):
                persisted_import = store.read_json("panel-import.json")
                if canonical_json_hash(persisted_import) != canonical_json_hash(
                    detached_snapshot
                ):
                    raise ConfigError(
                        "Stored panel import does not match the immutable continuation request"
                    )
            else:
                store.write_json("panel-import.json", detached_snapshot)
                store.mark_stage("panel-import", "completed", "panel-import.json")

            if store.exists("ledger.json"):
                budget.restore(store.read_json("ledger.json"))
            accounting_initialized = True
            # Persist the already-consumed source scope before the first child
            # provider dispatch. This prevents a failed continuation from
            # presenting its child-only ledger as the complete run cost.
            persist_continuation_accounting("in_progress")
            store.check_kill()

            reports = [dict(report) for report in reports_value]
            provenance_notice = {
                "continuation_source": dict(lineage),
                "receipt_scope": (
                    "Imported panel receipts remain bound to the source run; "
                    "this child run starts with the judge retry."
                ),
            }
            continuation_evidence = (
                mechanical_evidence.rstrip()
                + "\n\nIMMUTABLE CONTINUATION PROVENANCE\n"
                + json.dumps(provenance_notice, indent=2, sort_keys=True, ensure_ascii=False)
            ).strip()
            ...
            while not gate.get("passed") and amendment_round < max_amendments:
                ...
                prior_artifact_hash = str(
                    gate.get("artifact_sha256", text_hash(synthesis))
                )
                synthesis, _ = self._run_synthesis(
                    task,
                    context,
                ...
                if (
                    ...
                    gate = {
                        ...
                        "pass_count": 0,
                        "required_passes": int(
                            gate_config.get("required_passes", 1)
                        ),
                        "fail_closed": True,
                        "deterministic_blockers": [
                            "The amendment is byte-identical to the rejected artifact; "
                            "a fresh corrected artifact is required."
                        ],
                        "reviewers": [],
                    }
                    amendment_gate_name = f"gate-{amendment_round}.json"
            ...
            # Revalidate the exact source immediately before committing child
            # completion. A failed check marks only this child failed; the
            # source run is never opened for writes.
            validated_execution_source_snapshot(
                source_binding,
                reverify=True,
                task=task,
            ...
            status = "completed" if gate.get("passed") else "rejected"
            _child_ledger, aggregate_ledger = persist_continuation_accounting(
                "completed",
                child_outcome=status,
            )
            handoff = build_handoff(
            ...
            if accounting_initialized:
                refresh_failed_accounting("aborted", exc)
            store.finish("aborted")
            raise
        except Exception as exc:
            if accounting_initialized:
                refresh_failed_accounting("failed", exc)
            store.finish("failed")
            raise
        finally:
            store.close()
