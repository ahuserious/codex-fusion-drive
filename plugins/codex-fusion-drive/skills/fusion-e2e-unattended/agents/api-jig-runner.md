...
Kun Chen / kunchenguid AXI catalog has **no dedicated mock-API AXI** (gh-axi,
chrome-devtools-axi, lavish-axi, quota-axi, no-mistakes, backpass). Do not invent
a gh-axi mock server. Use skill `api-test-jig`: OpenAPI **Prism**
(`npx @stoplight/prism-cli mock`) or **MSW** local server with record/replay.
