...
def build_command(
    ...
    plugin_source = REPOSITORY_ROOT / "plugins" / "codex-fusion-drive"
    benchmark_support = BENCH_ROOT / "support"
    resolved_secret_file = secret_file or Path(
        "/private/tmp/codex-fusion-drive-xai"
    )
    tool_mounts = [
        {
            "type": "bind",
            "source": str(plugin_source),
            "target": "/opt/codex-fusion-drive",
            "read_only": True,
        },
        {
            "type": "bind",
            "source": str(benchmark_support),
            "target": "/opt/codex-fusion-drive-bench",
            "read_only": True,
        },
        {
            "type": "bind",
            "source": str(resolved_secret_file),
            "target": "/run/secrets/codex-fusion-drive-xai",
            "read_only": True,
        },
    ]
...
def resolve_xai_api_key() -> str:
    ...
    plugin_root = REPOSITORY_ROOT / "plugins" / "codex-fusion-drive"
...
def make_contract(
    ...
    return {
        ...
        "pins": {
            ...
            "ri_data_directory": "/logs/agent/codex-fusion-drive",
...
def build_evidence_index(attempt_directory: Path, contract: dict[str, Any]) -> Path:
    ...
    ri_root = trial_directory / "agent" / "codex-fusion-drive" / "runs"
    ri_runs = []
