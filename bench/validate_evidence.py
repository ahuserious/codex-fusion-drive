...
PLUGIN_ROOT = BENCH_ROOT.parent / "plugins" / "codex-fusion-drive"
...
def validate_contract(contract: dict[str, Any], index: dict[str, Any]) -> None:
    ...
    require(
        observed.get("ri_data_directory") == "/logs/agent/codex-fusion-drive",
    ...
    plugin_mount = mount_by_target.get("/opt/codex-fusion-drive")
    ...
    require(
        str(plugin_mount.get("source", "")).endswith("/plugins/codex-fusion-drive"),
        "Bad plugin source mount",
    )
    support_mount = mount_by_target.get("/opt/codex-fusion-drive-bench")
    ...
    secret_mount = mount_by_target.get("/run/secrets/codex-fusion-drive-xai")
...
def is_relentless_inception_item(item: Any) -> bool:
    ...
    return server == "codex-fusion-drive"
    ...
    for stage, expected_run_id in EXPECTED_RUN_IDS.items():
        ...
        if stage == "fuse":
            ...
            expected_artifacts_dir = (
                f"/logs/agent/codex-fusion-drive/runs/{expected_run_id}"
            )
            require(
